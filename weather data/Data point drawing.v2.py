...
# Annotate points with dates (to the right)
x_offset = 0.03 * (max(x) - min(x))
for i, date in enumerate(date_labels):
    ax.text(x[i] + x_offset, y[i], f'{date}', fontsize=10,
            ha='left', va='center', zorder=5)

# Annotate points with moisture (below the point, red color)
y_offset = 0.03 * (max(y) - min(y))
for i, date in enumerate(date_labels):
    moisture = moisture_values[date]
    ax.text(x[i], y[i] - y_offset, f'{moisture:.1f}%', fontsize=7,
            color='red', ha='center', va='top', zorder=5)
